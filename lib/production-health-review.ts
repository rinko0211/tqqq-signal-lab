...
export function assertProductionHealthLedgerInternalIntegrity(ledger:ProductionHealthLedger){
  if(ledger.schemaVersion!==1||ledger.appendOnly!==true)throw Error("Production Health prior ledger is invalid; refusing append-only reset");const generation=Date.parse(ledger.updatedAt);if(!Number.isFinite(generation))throw Error("Production Health integrity: invalid ledger generation updatedAt");const keys=new Set<string>(),logical=new Set<string>();let previousRecordedAt=-Infinity;for(const e of ledger.events){const id=`${e.version}|${e.dueDate}`;if(!e.key||e.key!==id||keys.has(e.key)||logical.has(id))throw Error(`Production Health integrity: duplicate/invalid event ${e.key}`);if(!isValidIsoMarketDate(e.dueDate)||!Number.isFinite(Date.parse(e.recordedAt)))throw Error(`Production Health integrity: invalid event date/timestamp ${e.key}`);const recordedAt=Date.parse(e.recordedAt);if(marketDate(e.recordedAt)<e.dueDate)throw Error(`Production Health integrity: review evidence predates claimed due date ${e.key}`);if(recordedAt>generation)throw Error(`Production Health integrity: event chronology exceeds ledger generation ${e.key}`);if(recordedAt<previousRecordedAt)throw Error(`Production Health integrity: out-of-order event chronology ${e.key}`);previousRecordedAt=recordedAt;keys.add(e.key);logical.add(id);}
