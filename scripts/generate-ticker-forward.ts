...
import { emptyTickerForwardLedger, TICKER_FORWARD_BUILD, type TickerForwardLedger } from "../lib/ticker-forward.ts";
...
try {
  ...
  await Promise.all([
    ...
    writeFile(new URL("ticker-forward-status.json", root), `${JSON.stringify({generatedAt,status:"success",ticker:"UPRO",version:"UPRO-VS13-v1.0",marketDataDate:ledger.records.at(-1)?.marketDataDate||null,records:ledger.records.length,dataSource:payload.source,buildVersion:process.env.GITHUB_SHA?.slice(0,12)||TICKER_FORWARD_BUILD,errors:[]},null,2)}\n`),
  ]);
} catch (error) {
