...
export function ResearchLineage() {
  const rows = [
    ["Phase 2", "Leverage Risk Control", "3x=13% / 2x=8.67%", "2xは単純な劣化版ではない"],
...
export function IntegratedDashboard({ production, productionForward, phase5, phase5Status }: {
    ...
    <>
      <section className="opsDashboard">
        <article className="decision productionCard">
          <div className="panelHead"><div><em>PRODUCTION · HUMAN CONTROLLED</em><h2>現在の実運用基準</h2></div><Chip tone="ok">PRIMARY</Chip></div>
          {production ? <>
          ...
          <div className="opsMiniGrid"><div><span>Forward資産</span><b>{champion ? yen(champion.currentCapital) : "—"}</b></div><div><span>Forward DD</span><b>{champion ? pct(champion.currentDd) : "—"}</b></div><div><span>Evidence</span><b>{champion?.evidence || "—"}</b></div></div>
        </article>
...
export function Phase5ForwardPanel({ productionForward, ledger, status }: {
  ...
  return (
    <>
      <article className="guideHero phase5Hero"><em>PHASE 5 · TRUE FORWARD GATE</em><h2>3候補を凍結して未来だけで検証</h2><p>2026-08-25より前へSignalを遡及しません。6か月は情報更新のみ、12か月で初めてPhase 6昇格可否を審査します。</p><div className="contextBadges"><Chip tone={status?.status === "success" ? "ok" : "bad"}>{status?.status === "success" ? "Workflow正常" : "Workflow要確認"}</Chip><Chip tone="warn">Production変更なし</Chip></div></article>
      ...
      <article className="panel"><div className="panelHead"><div><em>FORWARD FRONTIER</em><h2>既存TQQQ Championとの比較</h2></div><span>短期順位ではなく、壊れていないかを優先</span></div><SimpleTable heads={["System", "役割", "開始", "現在資産/Return", "Current DD", "Action", "Evidence", "次回Review"]} rows={[
...
export function Phase5PaperPanel({ ledger }: { ledger: Phase5Ledger | null }) {
  ...
  return (
    <article className="panel phase5Paper">
      ...
      <p className="note">既存Production Paperの初期資金・固定USD/JPY設定を共有します。ただし各候補の開始日は凍結された真のForward開始日から変更できません。3口座を同時保有する提案ではなく、比較用の独立仮想口座です。</p>
      ...
      {accounts.length > 0 && <>
        <SimpleTable heads={["Candidate", "役割", "開始日", "現在資産", "損益", "Return", "Max DD", "Action", "Target", "次回約定"]} rows={accounts.map((a) => [`${a.ticker} / ${a.version}`, roleLabel[a.role] || a.role, a.startDate, yen(a.equityJpy), yen(a.pnlJpy), pct(a.totalReturn), pct(a.maxDd), a.actions, `${Math.round(a.currentTarget * 100)}%`, a.nextExecutionDate || "なし"])} />
...
export function Phase5SystemStatus({ ledger, status }: { ledger: Phase5Ledger | null; status: Phase5StatusFile | null }) {
  ...
  const latest = status?.latestDates ? Object.values(status.latestDates).sort().at(-1) : undefined;
