...
export function LifecycleActionCenter(){
  ...
  return <>
    ...
    <article className="panel"><div className="panelHead"><div><em>WHAT TO DO NEXT</em><h2>今ユーザーがすること</h2></div><span>自動判定はしても自動売買戦略変更はしません。</span></div>
      ...
      {effectiveAction==="RUN_PHASE6_HUMAN_REVIEW"&&<><div className="opsNext"><span>STEP 1</span><strong>GitHub Actions → Human Production Approval → mode = DECISION を実行</strong></div><div className="opsNext"><span>STEP 2</span><strong>下のProduction-selectable候補から、現行Production維持を含め1本だけ選択</strong></div><div className="opsNext"><span>STEP 3</span><strong>同WorkflowをPRODUCTIONで実行し、Ticker / Strategy / Versionを下記どおり入力、confirmationに APPROVE PRODUCTION</strong></div></>}
    ...
    <article className="panel"><div className="panelHead"><div><em>REVIEW GATE</em><h2>4つのFrontierの自動判定</h2></div><span>Operationally eligible ≠ Production-selectable。最終変更はHuman Approvalのみ。</span></div>
      ...
      <p className="note">Eligibleは「壊れておらず正式レビューに載せられる」という意味です。Production-selectableはさらにcurrent incumbentとの共通期間Pareto比較を通った候補だけです。63観測未満のAction/yearは年率化ノイズのため表示しません。</p>
