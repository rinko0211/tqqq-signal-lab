...
export function LifecycleActionCenter(){
  ...
  const c=review.current,eligible=c.candidateReviews.filter(x=>x.eligible),effectiveAction=reviewStale?"CHECK_DATA_AND_ACTIONS":health?.current.userAction!=="NONE"?health!.current.userAction:c.userAction;
  ...
  return <>
    ...
    <article className="panel"><div className="panelHead"><div><em>REVIEW GATE</em><h2>4つのFrontierの自動判定</h2></div><span>EligibleでもHuman ApprovalまではResearchです。</span></div><div className="table"><table><thead><tr><th>System</th><th>Decision</th><th>Live obs</th><th>Executions</th><th>Regimes</th><th>Action/y</th><th>Max DD</th></tr></thead><tbody>{c.candidateReviews.map(x=><tr key={x.version}><td>{x.ticker}{x.incumbent?" · incumbent":""}<br/><small>{x.version}</small></td><td><span className={`status ${x.eligible?"ok":x.decision.includes("REVIEW")?"bad":"neutral"}`}>{x.decision}</span></td><td>{x.liveObservations}</td><td>{x.executions}</td><td>{x.regimes}</td><td>{Number.isFinite(x.actionDaysPerYear)?x.actionDaysPerYear.toFixed(1):"—"}</td><td>{(x.maxDd*100).toFixed(1)}%</td></tr>)}</tbody></table></div>{eligible.length>0&&!reviewStale&&<div className="formula"><p>Production入力候補:</p>{eligible.map(x=><code key={x.version}>Ticker: {x.ticker} / Strategy: {strategy[x.version]} / Version: {x.version}</code>)}</div>}</article>
