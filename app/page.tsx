...
export default function Home() {
  ...
  return (
    <main>
      ...
      <footer>
        <b>TQQQ Signal Lab v3 · Integrated Forward Gate</b>
        <span>
...
function ForwardMini({ledger}:{ledger:ForwardLedger}){
  ...
  return <article className="panel health"><div className="panelHead"><div><em>FORWARD VALIDATION · IMMUTABLE</em><h2>TQQQ Reference Baseline：VS13-v1.0</h2></div><Status kind="warn">{champ.evidence} Evidence</Status></div><section className="metrics compact"><Metric label="Forward Model指数" value={modelIndex(champ.totalReturn)} sub={`${pct(champ.totalReturn)} · 開始=100 / FX・税・実brokerコスト除外`}/><Metric label="現在DD" value={pct(champ.currentDd)}/><Metric label="注文" value={String(champ.orders)+"回"}/><Metric label="Legacy checkpoint" value={ledger.reviewSchedule.sixMonth} sub="正式GateはReview / 次のActionを参照"/></section><p className="note">まだForward期間が短いため順位は確定していません。Championの自動変更は行いません。金額はJPY-normalized比較用仮想元本で、FX・税・broker固有コストを含む実口座損益ではありません。</p></article>;
}

function ForwardView({ledger}:{ledger:ForwardLedger}){
  const rows=summarizeForward(ledger),champ=rows.find(x=>x.id==="VS13")!;
  return <>
    <article className="guideHero"><em>STRATEGY FORWARD TEST · APPEND ONLY</em><h2>未来データで戦略を正式比較</h2><p>2026-08-21以降、その日に公開されたSignalだけを保存します。後日の再計算で当時のSignalを上書きしません。</p><p className="warningNote">表示する円額はJPY-normalized比較用仮想元本です。USD ETF価格リターンを追跡し、FX・税・broker固有の実約定摩擦は含みません。</p></article>
    <section className="glossaryGrid">
      <article className="panel"><em>TQQQ REFERENCE BASELINE</em><h2>VS13 — バランス型Reference</h2><p>相場が悪化した時、保有後のTQQQ高値から13%下がることを防御条件の一つにします。成長力と下落抑制の釣り合いが最も安定していたため、現在の基準戦略です。</p><p className="note">平易に言えば「上昇には参加し、危険が強まれば段階的に現金へ逃げる」戦略です。</p></article>
...
function CrossTickerView({data,forward,forwardStatus}:{data:CrossBundle;forward:TickerForwardLedger|null;forwardStatus:{status:string;marketDataDate:string|null;records:number;version:string;errors:string[]}|null}){
  const forwardRows=forward?summarizeForward(forward):[];
  return <>
    ...
    <article className="panel"><em>UPRO FORWARD LEADERBOARD · FUTURE DATA ONLY</em><h2>CommonとNativeの実績比較</h2>{forwardRows.length?<Table heads={["Strategy","Version","現在資産","Total Return","現在DD","Max DD","Sortino","Calmar","注文","累積コスト","観測","Evidence","判定"]} rows={forwardRows.map(x=>[x.id==="UPRO_VS13"?"Common VS13":"Native VT25",x.version,yen(x.currentCapital),pct(x.totalReturn),pct(x.currentDd),pct(x.metrics.maxDd),x.observations<20?"—":num(x.metrics.sortino),x.observations<63?"—":num(x.metrics.calmar),x.orders,yen(x.transactionCosts),x.observations,x.evidence,x.id==="UPRO_VS13"?"COMMON BASELINE":x.status])}/>:<p>初回Forward記録を待っています。</p>}<p className="warningNote">これは2026-08-21以降に実際に保存された未来向き記録です。過去のOOS成績とは混ぜません。観測20日未満のSortino、63日未満のCalmarは表示しません。</p></article>
    {forward&&<section className="split"><article className="panel"><em>UPRO FORWARD EQUITY</em><h2>100万円を同時スタート</h2>{forward.freezes.map(f=>{const points=forward.records.filter(r=>r.strategyVersion===f.version).map(r=>({equity:r.equity}));return <div key={f.version}><strong>{f.version}</strong><Curve points={points} label={f.version+" Forward Equity"}/></div>})}</article><article className="panel"><em>UPRO UNDERWATER STATUS</em><h2>最高値からの下落</h2><Table heads={["Version","現在DD","最大DD","回復日数"]} rows={forwardRows.map(x=>[x.version,pct(x.currentDd),pct(x.metrics.maxDd),x.metrics.recoveryDays===null?"未回復":String(x.metrics.recoveryDays)+"日"])}/></article></section>}
