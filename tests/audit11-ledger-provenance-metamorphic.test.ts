...
function phase5Payload(){const base=Object.fromEntries(Object.entries(market.series).map(([t,rows])=>[t,clone(rows)])) as Record<string,any[]>;const source=(t:"SPY"|"QQQ",date:string)=>base[t].find(x=>x.date===date)?.close;for(const [ticker,version,proxy] of [["UPRO","UPRO-SPBT-v1.0","SPY"],["SSO","SSO-SPBT-Scaled-v1.0","SPY"],["QLD","QLD-VS13-Scaled-v1.0","QQQ"]] as const){const persisted=phase5.records.filter(r=>r.strategyVersion===version&&r.marketDataDate<fixtureHorizon);const basis=persisted.at(-1);const proxyClose=basis&&source(proxy,basis.marketDataDate);if(!basis||!proxyClose)throw new Error(`Audit 11 MR-05 fixture lacks a coherent ${version} basis`);base[ticker]=scaleBars(base[proxy],basis.assetClose/proxyClose);for(const record of persisted){const bar=base[ticker].find(x=>x.date===record.marketDataDate);if(!bar||!Number.isFinite(bar.close)||bar.close<=0)throw new Error(`Audit 11 MR-05 fixture lacks provider bar for ${version} ${record.marketDataDate}`);const k=record.assetClose/bar.close;bar.open*=k;bar.high*=k;bar.low*=k;bar.close*=k;bar.adjClose=(bar.adjClose??bar.close)*k}}return{source:"Audit11 stored market fixture",retrievedAt:fixtureGeneration,series:base}}
