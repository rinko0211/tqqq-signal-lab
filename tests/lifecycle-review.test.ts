...
const base=(now="2027-08-25T12:00:00Z")=>{const phase5=emptyPhase5Ledger("2026-08-25T15:15:00Z"),forward=emptyForwardLedger();return{phase5,forward,production:DEFAULT_PRODUCTION_CONFIG,phase5Status:{status:"success",errors:[],generatedAt:"2027-08-25T11:00:00Z",latestDates:{UPRO:"2027-08-24",SSO:"2027-08-24",QLD:"2027-08-24",SPY:"2027-08-24",QQQ:"2027-08-24",VIX:"2027-08-24"}},runtimeStatus:{generatedAt:"2027-08-25T11:00:00Z",actionStatus:"success",state:"latest",marketDataDate:"2027-08-24",errors:[]},now}};

test("six-month interim review never promotes",()=>{
  const x=base("2027-02-25T12:00:00Z");x.phase5Status.generatedAt="2027-02-25T11:00:00Z";for(const k of Object.keys(x.phase5Status.latestDates))x.phase5Status.latestDates[k]="2027-02-24";x.runtimeStatus.generatedAt="2027-02-25T11:00:00Z";x.runtimeStatus.marketDataDate="2027-02-24";
  for(const f of x.phase5.freezes)seedPhase5(x.phase5,f.version,130,6);seedIncumbent(x.forward,130,6);
...
test("formal review makes incumbent and non-dominated challengers human-selectable but never auto-produces",()=>{
  const x=base();for(const f of x.phase5.freezes)seedPhase5(x.phase5,f.version,260,6);seedIncumbent(x.forward,260,6);
  const r=updateLifecycleReview({...x,prior:emptyLifecycleLedger(x.phase5.reviewSchedule,"2026-08-25T00:00:00Z")});
...
test("uninterrupted bull labels cannot satisfy semantic regime coverage",()=>{
  const x=base();for(const f of x.phase5.freezes)seedPhase5(x.phase5,f.version,260,6,["強い上昇","弱い上昇"]);seedIncumbent(x.forward,260,6,["強い上昇","弱い上昇"]);
...
test("insufficient execution sample continues Forward when incumbent is also insufficient",()=>{
  const x=base();for(const f of x.phase5.freezes)seedPhase5(x.phase5,f.version,260,2);seedIncumbent(x.forward,260,2);
...
test("one failed challenger feed does not dead-end a healthy incumbent decision",()=>{
  const x=base();for(const f of x.phase5.freezes)seedPhase5(x.phase5,f.version,260,6);seedIncumbent(x.forward,260,6);x.phase5Status.status="failed";x.phase5Status.errors=["provider"];
...
test("stale/failed incumbent upstream status blocks all Production selection",()=>{
  const x=base();for(const f of x.phase5.freezes)seedPhase5(x.phase5,f.version,260,6);seedIncumbent(x.forward,260,6);x.runtimeStatus.generatedAt="2027-08-20T22:00:00Z";
