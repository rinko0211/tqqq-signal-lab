...
const phase5Payload=(end:string)=>{const dates=weekdaysEnding(end,320),v=dates.map(date=>({date,open:18,high:18,low:18,close:18,adjClose:18,volume:0}));return{source:"test",retrievedAt:"test",series:{UPRO:bars(dates,40,.0012),SSO:bars(dates,50,.0008),QLD:bars(dates,60,.0009),SPY:bars(dates,100,.0004),QQQ:bars(dates,120,.0006),VIX:v}} as any};
