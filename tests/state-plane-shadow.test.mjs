...
test("shadow state mirror is non-authoritative and cannot write main", async () => {
  ...
  for (const writer of [
    ...
    assert.ok(text.includes(writer), `missing workflow_run trigger: ${writer}`);
  }
...
test("source checkout is read-only while only ops-state checkout retains write credentials", async () => {
  const text = await readFile(path, "utf8");
  const sourceBlock = text.slice(
    text.indexOf("Checkout current authoritative main source"),
    text.indexOf("Checkout isolated ops-state target"),
  );
  const stateBlock = text.slice(
    text.indexOf("Checkout isolated ops-state target"),
    text.indexOf("Mirror operational state only"),
  );
